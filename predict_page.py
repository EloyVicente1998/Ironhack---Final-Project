...
def show_predict_page():
    ...
    orientation_dummy = [orientation in orientation_selection for orientation in orientation_columns]
    ...
    barrios_dummy = [barrio in selected_columns for barrio in dummy_columns]
    ...
    if ok:
        ...
        *orientation_dummy, HASBOXROOM, HASWARDROBE, HASSWIMMINGPOOL,
        HASDOORMAN, HASGARDEN, CADCONSTRUCTIONYEAR, DISTANCE_TO_CITY_CENTER,
        DISTANCE_TO_METRO, DISTANCE_TO_DIAGONAL, *barrios_dummy]])

        print(selected_columns)
        price = lasso_loaded.predict(X)
        st.subheader(f'The estimated price/m2 is €{price[0]:.2f}')
